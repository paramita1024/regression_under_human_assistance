...
class kl_triage:
    ...
    def get_subset(self,K):
		...
		# print 'machine err', self.machine_err.shape
		# print 'c', self.c.shape
		# print 'sqrt(c)', np.sqrt(self.c).shape
...
class Submodularity_ratio:
	...
	def get_k_l(self):
		...
		plt.show()
		print min(l_V_k)
		# print l_Null - min(l_V_k)
		# print l_Null
		# print '--'

		# x_norm =  np.array( [ LA.norm(self.X[i]) for i in self.V ]  )
		# print 'gamma_high', self.lamb/( self.lamb + np.max( x_norm ))
		# print 'gamma_low', np.max( [ float(self.c[i])/self.Y[i] ] )
...
class SubMod:
	...
	# 	# print xy.shape
	# 	# a = np.hstack(( np.array([[yTy + c_S]]) , xy.T) )
	# 	# b =  np.hstack((xy,B+xxT))
	# 	# M_f=np.vstack((a,b))
	# 	# return 0
	# 	M_f = np.vstack((np.hstack(( np.array([[yTy + c_S]]) , xy.T) ), np.hstack((xy,B+xxT)) ))
	# 	g_s=np.log( LA.det(B+xxT) )
	# 	f_s=np.log(LA.det(M_f))
	# 	print g_s - f_s
	# 	return g_s-f_s

	# def give_inc_new_det(self,elm,subset):
	# 	subset_curr = np.array([i for i in self.V if i in subset or i == elm])
	# 	X=self.X[subset_curr].T
	# 	Y=self.Y[subset_curr]
	# 	# yTy = self.yTy - y*y
	# 	# xxT = self.xxT - x.dot(x.T)
	# 	# xy=self.xy - y*x
	# 	c_S = self.c_S + self.c[elm]
	...
	# 	return -np.log( Y.dot(Y) - X_Y.T.dot( LA.inv( B+X.dot(X.T) ).dot(X_Y)  ) + c_S )
